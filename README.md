...
**Status geral:** stack completa funcionando — backend com CRUD, autenticação JWT e PostgreSQL; frontend com login/cadastro conectados, carrinho como checkout real (adicionar é local/reversível, "Finalizar" é quem registra o aluguel de verdade) e favoritos sincronizados com a API (persistem entre dispositivos, com merge automático do que foi favoritado como visitante) e edição dos próprios dados de conta; painel administrativo separado (`admin.html`), com login próprio, gráficos + relatórios de faturamento em SQL, gestão de clientes/vendas/produtos/devoluções e listas paginadas e com busca
