...
**Status geral:** stack completa funcionando — backend com CRUD, autenticação JWT e PostgreSQL; frontend com uma tela de boas-vindas (login/cadastro/continuar sem conta) antes do catálogo, carrinho como checkout real, carrinho como checkout real (adicionar é local/reversível, "Finalizar" é quem registra o aluguel de verdade) e favoritos sincronizados com a API (persistem entre dispositivos, com merge automático do que foi favoritado como visitante) e edição dos próprios dados de conta; painel administrativo separado (`admin.html`), com login próprio, gráficos + relatórios de faturamento em SQL, gestão de clientes/vendas/produtos/devoluções e listas paginadas e com busca

**Índice:** [Frontend](#--frontend-javascript-html-css) ([bug do carrinho/checkout](#bug-real-o-carrinho-não-era-um-carrinho--o-aluguel-já-tinha-acontecido-antes-dele)) · [Backend](#--backend-python) ([bug do ForeignKey](#bug-real-que-encontramos-e-corrigimos-foreignkey-sem-integridade-aplicada) · [hash/sal/JWT](#aprofundando-hash-de-senha-sal-e-jwt) · [acesso admin escondido](#aprofundando-por-que-acesso-escondido-não-pode-significar-url-secreta) · [bug do JOIN/select_from](#bug-real-dbqueryfuncavg-não-sabe-de-onde-partir-o-join) · [faturamento médio ambíguo](#aprofundando-por-que-faturamento-médio-sozinho-é-uma-pergunta-incompleta)) · [Banco de dados](#--banco-de-dados) · [Visão geral do produto](#visão-geral-do-produto-o-que-o-moviehustler-é)
...
└── src/
    ...
    ├── entrada.js                  # tela de boas-vindas antes do catálogo — Entrar/Criar conta/Continuar sem conta
...
- **Tela de boas-vindas antes do catálogo** (`entrada.js`): quem chega no site pela primeira vez (sem token salvo e sem ter passado por aqui antes) vê um overlay com "Entrar" / "Criar conta" / "Continuar sem conta" **antes** de conseguir mexer no catálogo por trás. Clicar em "Entrar" ou "Criar conta" fecha o overlay e abre o modal de conta já na aba certa (reaproveita `abrirConta()`, sem duplicar nenhum formulário); "Continuar sem conta" só fecha o overlay, mantendo o catálogo público e o fluxo de favoritos-visitante intactos. A escolha (de qualquer um dos três botões) fica salva no `localStorage` — o overlay não volta a aparecer nas próximas visitas desse navegador, mesmo sem login, então não vira uma cobrança repetida (ver "Aprofundando" abaixo)
...
- **Menu do perfil**: depois de logado, o botão vira um avatar + primeiro nome, que abre um menu com 5 opções — **Perfil** (nome/email da conta), **Pedidos** (histórico real de aluguéis, vindo de `GET /alugueis/`), **Favoritos** (painel próprio com os filmes favoritados), **Configurações** (tema claro/escuro/sistema, e um formulário pra editar nome/e-mail/senha da própria conta — `PUT /auth/me`) e **Sair**
- Aluguel real, protegido por login: escolher agência e clicar "Alugar agora" adiciona o filme ao carrinho (só local, reversível); é o botão **"Finalizar aluguel" do carrinho** que de fato chama `POST /alugueis/`, um item de cada vez, com o token no header `Authorization`. Se a pessoa não estiver logada, o clique em "Alugar agora" abre o modal de login em vez de adicionar ao carrinho (o botão já avisa antes, mostrando "Entrar para alugar")
...
- **Painel administrativo** (`admin.html`, separado do site do cliente — ver ["Aprofundando: por que 'acesso escondido' não pode significar 'URL secreta'"](#aprofundando-por-que-acesso-escondido-não-pode-significar-url-secreta) na seção de Backend): login próprio (mesma API de autenticação, mas rejeita quem não é admin), com 5 abas —
  - **Visão geral**: os 3 relatórios pedidos — faturamento por mês, filmes mais alugados e faturamento médio (que na prática virou **dois** números, ver "Aprofundando" na seção de Backend) — consumindo `/admin/relatorios/*`, com **gráfico de verdade** em cima de cada um (linha pro faturamento mensal, barra horizontal pros filmes mais alugados, via [Chart.js](https://www.chartjs.org/) carregado por CDN) além da tabela
  - **Clientes**: lista de usuários cadastrados (`GET /admin/clientes/`), **paginada** (10 por página, com "Anterior"/"Próxima" — ver "Aprofundando" na seção de Backend) e com **busca** por nome ou e-mail (campo com *debounce*, ver "Conceitos de JavaScript" abaixo)
  - **Vendas**: todo o histórico de aluguéis de todo mundo (não só o do usuário logado, diferente de "Pedidos" no site do cliente), com o nome do cliente, um botão **Marcar devolução** por linha ainda não devolvida, também **paginada** e com **busca** por filme ou cliente
  - **Produtos**: lista de filmes com o mesmo formulário fazendo dupla função — **criar** (`POST /filmes/`) e **editar** (`PUT /filmes/{id}`, clicando em "Editar" numa linha existente pré-preenche os campos e troca o formulário pra modo edição, com "Cancelar edição" pra voltar) — além de remover (`DELETE /filmes/{id}`). Reaproveita as mesmas rotas do catálogo público, agora protegidas por admin
  - **Devoluções**: histórico de tudo que já foi devolvido, com data do aluguel e da devolução lado a lado
- **Sessão expirando de forma amigável**: antes, se o token JWT vencesse (1 dia) enquanto alguém navegava, a próxima ação protegida simplesmente falhava com um erro genérico, sem reabrir o login. Agora toda chamada autenticada passa por `fetchAutenticado()` (`auth.js`), que detecta `401` sozinha, limpa a sessão e dispara um evento (`sessao-expirada`) — tanto o site do cliente quanto o painel admin escutam esse evento e reagem sozinhos: fecham o que estiver aberto, voltam pro estado deslogado e reabrem o login com "Sua sessão expirou. Faça login novamente." (ver "Conceitos de JavaScript estudados" abaixo pro porquê de ser um evento, não um callback)
...
- **Sessão simulada com `localStorage`**: como não existe "sessão de servidor" (o JWT é stateless — ver seção de Backend), guardar o usuário logado é responsabilidade só do navegador. `estaLogado()` é simplesmente `Boolean(obterToken())` — nenhuma chamada à API, só verifica se existe alguma coisa salva localmente. Isso é rápido, mas tem uma limitação real: se o token expirar no servidor, `estaLogado()` continua devolvendo `true` (o token ainda está salvo, só que vencido) — o frontend só descobre a expiração de verdade quando tenta *usar* o token numa chamada protegida e recebe `401`. É exatamente esse `401` que `fetchAutenticado()` intercepta (ver "Sessão expirando de forma amigável" abaixo).
...
- **"Login wall" com fuga, não sem saída**: pedir login antes de deixar a pessoa ver qualquer coisa é um padrão de UX com nome (*login wall*) e uma reputação ruim documentada — pesquisando antes de implementar, a razão central é que a barreira interrompe alguém que só queria "dar uma olhada" antes de decidir se confia no site o bastante pra criar uma conta, e uma fração real desses visitantes simplesmente vai embora em vez de se cadastrar. A recomendação que aparece de forma consistente na pesquisa de UX não é "nunca pedir login antes", é **sempre dar uma saída visível** quando a barreira não for estritamente necessária pro que a pessoa veio fazer (aqui, só *olhar* o catálogo não exige conta — só alugar e favoritar de verdade exigem). É por isso que "Continuar sem conta" tem o mesmo peso visual que os outros dois botões, não é um link escondido no rodapé — e por isso os três botões, não só o de pular, marcam a escolha como "já vi essa tela" no `localStorage`: a pessoa que ativamente escolheu "Entrar" mas desistiu no meio do caminho não devia ser recebida pela mesma pergunta de novo na próxima visita.

  *Fonte consultada: [Nielsen Norman Group — Login Walls Stop Users in Their Tracks](https://www.nngroup.com/articles/login-walls/).*
...
- **Sessão expirando de forma amigável, com um evento em vez de mais um callback**: antes dessa mudança, cada módulo (`movies.js`, `favoritos.js`, `adminApi.js`) montava o header `Authorization` na mão e nenhum tratava `401` de propósito — o erro só aparecia genérico onde a chamada tinha sido feita. A correção juntou duas coisas: (1) uma função só, `fetchAutenticado()` em `auth.js`, virou o único lugar que anexa o token e confere `resposta.status === 401` — todas as chamadas autenticadas do projeto passaram a rodar por ela; (2) quando `401` acontece, em vez de `auth.js` precisar *conhecer* `perfil.js` (site do cliente) e `admin.js` (painel) pra avisar os dois, ela só dispara `window.dispatchEvent(new Event("sessao-expirada"))` — um evento comum do DOM, sem nenhuma biblioteca. `perfil.js` e `admin.js`, cada um no seu canto, têm um `window.addEventListener("sessao-expirada", ...)` reagindo do jeito que faz sentido pra cada tela.

  Isso é uma variação do padrão de callback que já aparece em outros pontos do projeto (`conta.js`→`perfil.js`, `favoritos.js`→`catalogo.js`) — mas ali era **um** produtor avisando **um** consumidor, então passar uma função bastava. Aqui são **três** módulos que podem disparar o sinal e **duas** telas completamente diferentes que precisam reagir; conectar isso com callbacks exigiria `auth.js` importar (ou receber por parâmetro) tanto `perfil.js` quanto `admin.js`, quebrando a separação que o projeto guarda desde o início (módulos de dados não conhecem módulos de tela). Pesquisando o nome desse padrão pra confirmar que a escolha fazia sentido — é chamado de **pub/sub via eventos customizados do DOM** — a vantagem central que a literatura aponta é exatamente essa: publicador e assinante não precisam se conhecer, só concordar no nome do evento. `CustomEvent`/`dispatchEvent` são API nativa do navegador, sem framework nenhum envolvido.

  *Fontes consultadas: [MDN — CustomEvent](https://developer.mozilla.org/en-US/docs/Web/API/CustomEvent) e [serhii.io — How Custom Events in JavaScript Can Decouple Your Code](https://serhii.io/posts/how-custom-events-in-javascript-can-decouple-your-code).*
- **Canvas não é DOM — por que os gráficos precisam de cor "na mão"**: os dois gráficos da Visão geral (Chart.js) são desenhados dentro de um `<canvas>`, e o conteúdo desenhado ali **não faz parte da árvore do DOM** — é só um bitmap. Isso significa que nenhuma regra do `style.css` (incluindo os tokens de tema claro/escuro que o resto do projeto usa, tipo `--cor-texto-suave`) chega a afetar o que é pintado dentro do canvas sozinha; o Chart.js não tem ideia do que é `var(--cor-acento)`. A solução (função `corTema()` em `admin.js`) usa `getComputedStyle(document.documentElement).getPropertyValue("--cor-acento")` pra **ler o valor resolvido** daquela variável CSS em JavaScript, na hora de montar o gráfico, e passar isso como uma cor literal nas opções do Chart.js. Resultado prático: os gráficos já respeitam o tema claro/escuro do resto do site, mesmo sem nenhuma regra de CSS os alcançar diretamente — só porque leem a mesma fonte de verdade (os tokens) por um caminho diferente (JS, não cascata CSS).

  *Fontes consultadas: [MDN — Using CSS custom properties](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties), sobre ler variáveis CSS via `getPropertyValue`, e a [seção de `<canvas>` da especificação HTML (WHATWG)](https://html.spec.whatwg.org/multipage/canvas.html), que confirma que o conteúdo desenhado num canvas não integra a árvore do documento.*
- **Por que um `<form>` não manda `PUT` sozinho**: a primeira tentativa ingênua seria dar ao formulário de edição um `method="PUT"` e deixar o navegador cuidar do resto, do jeito que `<form method="POST">` já funciona nativamente em qualquer lugar do projeto. Pesquisando isso pra confirmar antes de escrever o código, a resposta é categórica: o atributo `method` de um `<form>` HTML **só aceita `GET` e `POST`** — mesmo escrevendo `method="PUT"` no HTML, o navegador ignora e manda `GET` por baixo. Essa limitação existe desde a especificação original do HTML e nunca mudou (há uma proposta recente, de 2025, discutindo adicionar `PUT`/`PATCH`/`DELETE` como valores válidos, mas ainda não é padrão em nenhum navegador). É por isso que o formulário de filme — como todo outro formulário do projeto — usa `evento.preventDefault()` pra impedir o envio nativo, e faz a requisição de verdade via `fetch()`, escolhendo `POST` ou `PUT` como uma opção de JavaScript, não do HTML.
- **Um formulário só, dois modos**: em vez de duas telas separadas ("criar filme" e "editar filme"), o painel Produtos reaproveita o mesmo `<form>` pras duas ações. `form.dataset.editando` guarda o `id` do filme em edição (ou fica ausente, em modo criação); no `submit`, essa única checagem decide entre chamar `criarFilme()` ou `atualizarFilme(id, ...)`. Clicar em "Editar" só chama `preencherFormFilme()` (que copia os campos do filme pros `<input>`s existentes) e marca `dataset.editando` — nenhum HTML novo é criado, é o mesmo formulário mudando de propósito. Pesquisando esse padrão (formulário único reaproveitado entre criar/editar é comum o bastante pra ter discussão própria na literatura de front-end), o conselho recorrente é tratar o formulário como uma **camada de exibição** cujo comportamento vem de fora, em vez de espalhar `if`s de "sou criação ou edição?" pelo meio da lógica de validação — é exatamente o papel que `dataset.editando` cumpre aqui: o **estado** de qual modo está ativo mora no próprio elemento do DOM (não numa variável solta do módulo), então não existe como o formulário "esquecer" em que modo está, mesmo sendo recriado do zero a cada `renderizarProdutos()`. O custo dessa escolha é precisar "resetar" esse estado explicitamente (`sairModoEdicao`) quando a edição é cancelada ou concluída — sem isso, o formulário ficaria preso em modo edição pra sempre.

  *Fontes consultadas: [WHATWG — proposta de suporte a PUT/PATCH/DELETE em `<form>`](https://github.com/whatwg/html/pull/11347) e [Alexander Petros — Support PUT, PATCH, and DELETE in HTML Forms](https://alexanderpetros.com/triptych/form-http-methods), sobre a limitação histórica do atributo `method`; [Nette — Reusing Forms in Multiple Places](https://doc.nette.org/en/best-practices/form-reuse), sobre tratar formulários reaproveitados como camada de exibição desacoplada do modo de operação.*

- **`debounce()` — esperar a pessoa parar de digitar antes de perguntar pro servidor**: a busca de Clientes/Vendas no admin dispara uma chamada de rede a cada busca — sem cuidado, isso significa uma requisição por **tecla digitada** (buscar "checkout" seria 8 chamadas, a maioria descartada antes de a resposta nem chegar). `debounce()` (`admin.js`) resolve isso com um padrão clássico: a cada tecla, cancela um temporizador pendente (`clearTimeout`) e agenda um novo (`setTimeout`); a função de busca de verdade só roda quando **350ms se passam sem nenhuma tecla nova** — ou seja, quando a pessoa dá uma pausa real, não a cada caractere. O nome vem daí: "tirar o ruído" de vários eventos próximos, deixando só o último valer.
- **Bug evitado antes de existir: o re-render "comendo" o foco do campo de busca**: o padrão do projeto inteiro é recriar o HTML do zero a cada mudança de estado (`conteudo.innerHTML = ...`, documentado desde o capítulo 8 do guia de JS). Isso é ótimo pra maioria das telas, mas quebra exatamente num campo de texto que a pessoa está digitando: o re-render troca o `<input>` antigo por um **elemento novo**, e o navegador não tem como saber que esse elemento novo "é o mesmo" pro efeito de manter o cursor piscando nele — o foco simplesmente se perde a cada busca, obrigando a pessoa a clicar de volta no campo pra continuar digitando. Antes de considerar a busca pronta, testei digitando palavra por palavra e percebi exatamente isso. A correção (`ligarBusca()`) chama `campo.focus()` e `campo.setSelectionRange(...)` **depois** do re-render terminar, no elemento novo que acabou de ser criado — restaurando o foco e posicionando o cursor no fim do texto, como se nada tivesse acontecido.

  *Fontes consultadas: [freeCodeCamp — How to Optimize Search in JavaScript with Debouncing](https://www.freecodecamp.org/news/optimize-search-in-javascript-with-debouncing/).*
...
#### Bug real: o "carrinho" não era um carrinho — o aluguel já tinha acontecido antes dele

Revisando o código pra decidir como fazer o carrinho "finalizar de verdade", apareceu um problema mais fundamental do que só "falta implementar": clicar em **"Alugar agora"** no popup do filme já chamava `POST /alugueis/` e registrava o aluguel no banco **na hora** — antes mesmo do item aparecer no carrinho. O botão "Finalizar aluguel" do carrinho, então, não tinha nada de fato pra fazer: o aluguel já tinha sido concluído um passo antes. Ele só limpava a lista visual e mostrava um alerta de sucesso — um checkout **decorativo**, que nunca falhava porque nunca fazia nada.

Pesquisando o motivo de isso ser considerado errado (não só "incompleto"), a prática padrão de e-commerce é clara sobre onde fica o ponto de compromisso: **"adicionar ao carrinho" nunca deve comprometer a transação** — item pode ser removido, é só intenção, sem custo pra desfazer. É o **checkout** ("Finalizar compra") quem carrega o clique de maior compromisso do fluxo inteiro, o único que deveria criar o registro definitivo. O projeto tinha essas duas etapas invertidas: o compromisso (criar o aluguel) acontecia no clique de baixo risco ("Alugar agora", que parece só uma preview), e o clique de alto risco ("Finalizar aluguel") não fazia nada.

A correção trocou os dois papéis:

- **`modal.js` — "Alugar agora"** agora só chama `adicionarAoCarrinho()`, sem tocar a API. Reversível, sem custo — clicar e depois remover do carrinho não deixa rastro nenhum no backend.
- **`carrinho.js` — "Finalizar aluguel"** agora percorre os itens e chama `criarAluguel()` um por um, de verdade. Cada item guarda o `filmeId` e o `agenciaId` (antes só guardava nome/preço pra exibir, informação insuficiente pra fazer a chamada real).
- **Falha parcial é tratada, não ignorada**: se um item falhar (ex: sessão expirou no meio do checkout, ou a agência escolhida deixou de existir), ele continua no carrinho pra tentar de novo — os que já foram registrados com sucesso saem da lista, sem duplicar caso o clique em "Finalizar" seja repetido.

*Fonte consultada: [Stripe — "Add to cart" explained: what it means and why it matters](https://stripe.com/resources/more/add-to-cart-explained), sobre por que adicionar ao carrinho não deve confirmar disponibilidade nem comprometer a transação, reservada só pro checkout.*
...
│   │   ├── paginacao.py           # Pagina[T] — envelope genérico reaproveitado por /admin/clientes e /admin/vendas
...
- `GET /admin/clientes/?pagina=1&por_pagina=10&busca=` — lista os usuários cadastrados, **paginado** (`pagina` ≥ 1, `por_pagina` entre 1 e 100 — validado pelo próprio FastAPI via `Query(..., ge=1, le=100)`, `422` se sair da faixa) e opcionalmente filtrado por `busca` (substring, sem diferenciar maiúsculas/minúsculas, batendo em nome **ou** e-mail)
- `GET /admin/vendas/?pagina=1&por_pagina=10&busca=` — histórico de aluguéis de **todos** os usuários (não só o do token, diferente de `GET /alugueis/`), cada um já com o cliente que alugou (`VendaOut`, que estende `AluguelOut` com o campo `usuario`), também paginado e filtrável por `busca` (título do filme **ou** nome do cliente)
- As duas rotas paginadas devolvem o mesmo formato de envelope — `Pagina[T]` (`schemas/paginacao.py`): `{ itens, total, pagina, por_pagina, total_paginas }` — ver "Aprofundando" abaixo
- A busca usa `ILIKE` (case-insensitive no Postgres) com `or_()` do SQLAlchemy pra "bate no campo A **ou** no campo B" — em `/admin/vendas/`, isso exige `JOIN` explícito com `Filme` e `Usuario` (a rota já não filtra só por colunas de `Aluguel`, precisa alcançar colunas de tabelas relacionadas antes mesmo de rodar o filtro)
...
- `PUT /auth/me` — **requer login** — atualiza nome/e-mail/senha da própria conta (nunca de outra — `usuario_atual` vem do token, não de um `id` na URL). `senha` é opcional (manda só se quiser trocar); `nome`/`email` são obrigatórios mesmo sem mudar, porque é um `PUT` de verdade — substitui o recurso inteiro, no mesmo espírito de `PUT /filmes/{id}` (ver "Aprofundando" sobre verbos HTTP). 409 se o novo e-mail já pertencer a outra conta
...
- **`Pagina[T]` — um schema genérico com `Generic`/`TypeVar`**: antes da paginação, cada rota de listagem tinha seu `response_model=list[Algo]` próprio. Envelopar a resposta paginada (`itens`, `total`, `pagina`, `por_pagina`, `total_paginas`) do mesmo jeito pra `/admin/clientes/` e `/admin/vendas/` criaria dois schemas quase idênticos, só trocando o tipo de `itens`. `schemas/paginacao.py` resolve isso com **um schema genérico**: `class Pagina(BaseModel, Generic[T])`, onde `T = TypeVar("T")` é um "tipo qualquer, a ser decidido depois". Cada rota declara `response_model=Pagina[UsuarioOut]` ou `response_model=Pagina[VendaOut]` — o FastAPI/Pydantic resolve `T` naquele ponto específico e gera (e documenta no Swagger) um schema concreto pra cada combinação, sem precisar escrever `PaginaDeClientes` e `PaginaDeVendas` na mão. É o mesmo princípio genérico de `list[FilmeOut]` que o projeto já usava — só que aqui é a gente criando o "molde reaproveitável", não usando um pronto do Python.
- **Paginação por página numerada (`offset`/`limit`), não por cursor**: a função `paginar()` usa `.offset((pagina - 1) * por_pagina).limit(por_pagina)` — pedir "a página 2" vira, em SQL, "pule os N primeiros registros, pegue os próximos N". É a forma mais simples de paginar, mas pesquisando as alternativas antes de escolher, existe uma outra abordagem (paginação por **cursor**, usada por APIs que lidam com volume grande, tipo redes sociais) que não usa números de página — ela pede "os itens depois deste marcador específico". A diferença prática: `OFFSET` obriga o banco a *contar e pular* todas as linhas anteriores a cada consulta, o que fica visivelmente mais lento à medida que a página cresce (rápido nas primeiras páginas, ruim numa página 500); cursor não tem esse problema porque nunca "pula" nada, só continua de onde parou. Em compensação, cursor não permite pular direto pra "página 47" nem mostrar o total de páginas com facilidade — coisas que `Pagina[T]` já devolve de graça. Pra um painel admin com algumas dezenas de clientes/vendas, a simplicidade do offset venceu de longe a escalabilidade do cursor — é a mesma lógica de custo-benefício já registrada antes neste README pro N+1 queries de `alugueis`: o problema é real, mas só importa na escala que esse projeto não tem.

  *Fontes consultadas: [Pydantic Generic Models — Orchestra](https://www.getorchestra.io/guides/pydantic-generic-models-a-comprehensive-tutorial-for-fastapi-users) e [FastAPI — Response Model](https://fastapi.tiangolo.com/tutorial/response-model/), sobre `Generic`/`TypeVar` em schemas; [Keyhole Software — Cursor-Based Pagination vs Offset Pagination](https://keyholesoftware.com/?p=160296), sobre o trade-off de performance entre as duas abordagens.*
...
- O painel admin cobre o pedido original (clientes, vendas, produtos com CRUD completo, devoluções, os 3 relatórios com gráfico e um login escondido), com Clientes e Vendas já paginados e com busca
